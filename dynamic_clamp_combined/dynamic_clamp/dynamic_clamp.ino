...
//            the lookup table numbers for potassium activation/inactivation) are calculated by calling the function
...
//            and the function called at every time step to calculate the potassium current to be injected. 
//
...
float gK_PM = 0.0;                // nS, maximal fast, transient potassium PM conductance
float gK_PN = 0.0;                // nS, maximal fast, transient potassium PN conductance
...
void setup() {
  ...
  GeneratePotassiumPMLUT();                     // generate potassium PM activation/inactivation lookup table
  GeneratePotassiumPNLUT();                     // generate potassium PN activation/inactivation lookup table
...
void loop() {
    ...
    if (gK_PM > 0){
        injectionCurrent += PotassiumPM(v);
    }
    if (gK_PN > 0) {
        injectionCurrent += PotassiumPN(v);
    }
